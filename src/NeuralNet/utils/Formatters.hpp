 ...
 * do the necessary adjustments on its parameters
 *
 ...
 */
template <typename T>
static Eigen::MatrixXd formatLabels(std::vector<T> labels,
  ...
  assert(labels.size() == rows &&
         "The number of labels don't match the number of inputs");
