...
class Tensor {
 public:
  ...
  /**
   * @brief This method will separate the data into batches of the specified
   * size
   *
  ...
  void batch(int batchSize) {
    ...
    for (int i = 0; i < numBatches; ++i) {
      ...
      batches.emplace_back(std::make_move_iterator(data.begin() + startIdx),
                           std::make_move_iterator(data.begin() + endIdx));
    }
  ...
  size_t size() const { return batched ? batches.size() : data.size(); };

  std::vector<T> getBatchedData() const { return batches; }
